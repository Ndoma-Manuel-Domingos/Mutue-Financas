  ...
  <MainLayouts>
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-uppercase">Mensalidades (Preçarios)</h1>
          </div>
          <div class="col-sm-6">
                  ...
                  <div class="form-group">
                    ...
                    <div class="input-group input-group">
                      ...
                      >
                        ...
                        <option :value="ano.id" v-for="ano in polos" :key="ano.id">{{ ano.designacao }}</option>
                      </select>
                    </div>
                  </div>
                </div>
                ...
                <div class="table-responsive">
                  <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td>
                          <input type="text" v-model="search_descricao" class="form-control"/>
                        </td>
                      </tr>
                      ...
                      <tr v-for="item in servicos.data" :key="item.Codigo">
                        ...
                        <td><span class="tag tag-success">AOA {{ item.Preco }}</span></td>
                        <td>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form id="formEditarTipoServico" @submit.prevent="submit">
            ...
            <div class="form-group mb-3">
              ...
              <select name="" id="" v-model="form.polo_id" type="text" class="form-control mb-1" :class="{'is-invalid': form.errors.polo_id }">
                <option value="">Todos</option>
                <option :value="ano.id" v-for="ano in polos" :key="ano.id">{{ ano.designacao }}</option>
              </select>
              <span class="text-danger d-block">{{ form.errors.polo_id }}</span>
            </div>
        ...
        <div class="modal-footer justify-content-between">
          <button type="button" class="btn-sm btn-default" @click="closeModel" >
            Fechar
          </button>
          <button type="submit" form="formEditarTipoServico" class="btn-sm btn-primary">
            Salvar
          </button>
        </div>
      </div>
...
<script>  
  ...
  export default {
    ...
    computed: {
      formTitle() {
        return this.isUpdate ? "Editar Mensalidades (Preçario)" : "Adicionar Mensalidades (Preçario)";
      }
    },
    watch: {
    ...
    methods: {
      ...
      formatValor(atual){
        const valorFormatado = Intl.NumberFormat('pt-br', {style: 'currency', currency: 'AOA'}).format(atual)
        return valorFormatado
      },
                  
      imprimirPDF() {
        window.open("/propinas/pdf-imprimir?a"+this.anolectivo+"&p="+this.polo, "_blank");
      },
